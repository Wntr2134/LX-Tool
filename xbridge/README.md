...
1. **Surface into MC mode.** X-Touch: power on holding channel-1 SELECT,
   set *MC* and *USB*. X32/M32: Setup → Remote → Mackie MCU, Card MIDI,
   REMOTE BUTTON enabled — then press **DAW REMOTE** on the top panel.
2. **Open XBridge**, pick your **Target** and **Surface**, press
   **Start bridge**. The status pill goes green and the port is named.
3. **Console setup…** prints the exact OSC lines to create, with your
   ports already in them. Type them into the console.
4. **Move each fader once.** Each one teaches the bridge which console
   object drives its motor. That is the whole feedback setup.
5. **Learn anything else.** *Learn a control…*, press the button or
   fader, say what it should do, Save — then move it once if it is a
   fader.

If a fader does not reach the console, press **Find MA3 format** and
watch the executor: it sweeps every dialect and keeps the one that
lands. If nothing at all arrives, it is a console switch — see
[The faders move but the console does not](#the-faders-move-but-the-console-does-not).
