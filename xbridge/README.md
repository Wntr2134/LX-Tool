...
| **Behringer X32 / Midas M32** (DAW-remote MC mode) | The console itself as a control surface. Setup → Remote: encoder 1 enables remote control, encoder 2 selects **Mackie Control** (not HUI, not raw CC), then pick the interface: **Card MIDI** (easiest — it appears as an ordinary USB MIDI port). Card MIDI means the **X-USB card's** USB-B socket. The X32 has *two* USB-B sockets on the back and the other one, **REMOTE**, is for X32-Edit remote control and carries no MIDI at all — plugged into that one the PC sees no MIDI inputs whatsoever. The X-USB driver must be installed on the PC as well, **MIDI In/Out** (DIN, needs a MIDI interface) or **RTPMIDI** (network; on Windows you need an rtpMIDI driver, and the port takes your session's name so pick it by hand). It is Mackie Control, so faders, SELECT/MUTE, encoders and the main fader all behave as the X-Touch does. Two differences: the X32 draws its own channel names so the scribble-strip SysEx is skipped, and it may not send fader-touch — in which case nothing is ever "held" and motor feedback simply applies |
